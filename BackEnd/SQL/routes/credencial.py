...
def get_credenciales(db: Session = Depends(get_db)):
    try:
        ...
        credenciales_list = [Credencial.model_validate(dict(row._mapping)) for row in result]
        return credenciales_list
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@credencial_routes.get("/credenciales/id/{credencial_id}", tags=["Gestión de credenciales"])
def get_credencial_by_id(credencial_id: str, db: Session = Depends(get_db)):
    try:
        ...
        return Credencial.model_validate(dict(result._mapping))
...
@credencial_routes.get("/credenciales/did/{did}", tags=["Gestión de credenciales"])
def get_credenciales_por_did(did: str, db: Session = Depends(get_db)):
    try:
        ...
        credenciales_list = [Credencial.model_validate(dict(row._mapping)) for row in cred_result]
        return credenciales_list
...
def insertar_credencial(credencial_data: Credencial, db: Session = Depends(get_db)):
    try:
        new_credencial = credencial_data.model_dump()
        result = db.execute(credencial.insert().values(new_credencial))
        db.commit()
        return {
            "status": "Credencial insertada",
            "credencial_id": result.inserted_primary_key[0] if result.inserted_primary_key else None
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def eliminar_credencial(id: str, db: Session = Depends(get_db)):
    try:
        ...
        # Actualizar credenciales del estudiante
        ...
        db.commit()
        return {
            "status": "Credencial y solicitudes asociadas eliminadas y estudiante actualizado",
            "id": id,
            "estudiante_nia": estudiante_nia
        }
